// GRAHAM ($GRAHAM) — a NAV-backed reserve protocol, recreation + enhancement of NetNet Capital.
//
// The mechanism, faithful to the source:
//  • Every GRAHAM is backed by at least 1 USDC of risk-free treasury value. NAV = treasury / supply.
...
//  • Stake GRAHAM -> sGRAHAM; the index compounds every epoch. Redemption is 1:1 and immediate.
...
//  • Loopback (Lombard): borrow USDC against sGRAHAM and recompound — a leverage loop (simulated here).
//
...
const TOKEN = process.env.TOKEN_TICKER || 'GRAHAM';
const DATA_PATH = process.env.DATA_PATH || path.join(ROOT, 'data.json');
const GRAHAM_MINT = process.env.GRAHAM_MINT || '';  // $GRAHAM on Base
// staking custody: enrolled $GRAHAM is held here (key kept offline by the operator — never on this server).
// Inert until GRAHAM_MINT is set; once launched, enrolling requires a real on-chain transfer to this wallet.
const TREASURY_WALLET = process.env.TREASURY_WALLET || '0xE662Beb1903884213720F35aeA92C75417b26442';
// Base RPC — read real on-chain $GRAHAM balances so the app shows the truth.
...
const LOOP_LTV = +(process.env.LOOP_LTV || 0.5);           // borrow up to 50% of sGRAHAM value
const LOOP_APR = +(process.env.LOOP_APR || 0.12);          // 12% APR on borrowed USDC
...
// each epoch, wallets earn bonus sGRAHAM on top of the base rebase, scaled by their tenure multiplier.
const LADDER = [
...
// ---- THE RESERVE BOOK ---- the treasury is held as tokenized real-world assets on Base.
...
const RESERVE = [
  ...
  { sym: 'USDC',  name: 'USDC reserve',  kind: 'stable', w: 0.18, y: 0.045 },
];
...
// ---- state ----
let db = {
  supply: +(process.env.SEED_SUPPLY || 200000),           // total GRAHAM
  treasury: +(process.env.SEED_TREASURY || 214000),       // risk-free USDC reserves
  marketPrice: +(process.env.SEED_PRICE || 90.7878),       // USDC per GRAHAM (TWAP stand-in)
...
for (const w of Object.values(db.wallets)) if (w.seeded && w.usdc <= 250000) w.usdc += SEED_USDC - 250000;
...
const SEED_USDC = +(process.env.SEED_USDC || 1e9);         // starting USDC — no perceived cap on how much you can buy/enroll

// live pool state (declared early: distribute/catchup run at load)
let poolPrice = 0, poolMcap = 0, poolLiq = 0, hasPool = false;
// ---- core math ----
function nav() { return db.treasury / db.supply; }                       // USDC per GRAHAM (treasury-backed)
...
function distribute() {
  ...
  const minted = db.totalAgons * db.index * rate;          // new GRAHAM owed to stakers this epoch
  ...
  // Seniority Ladder: tenured wallets earn BONUS sGRAHAM on top of the base rebase.
...
// ---- live market data ---- once $GRAHAM is live, mirror the real pool price/mcap/supply from DexScreener,
...
async function pollDex() {
  if (!GRAHAM_MINT) return;
  try {
    const r = await fetch('https://api.dexscreener.com/latest/dex/tokens/' + GRAHAM_MINT, { headers: { accept: 'application/json' } });
    if (!r.ok) return;
...
function W(a) { return db.wallets[a] || (db.wallets[a] = { usdc: SEED_USDC, graham: 0, agons: 0, bonds: [], loan: 0, seeded: true }); }
...
function metrics() {
  ...
  return {
    token: TOKEN, mint: GRAHAM_MINT, epochSec: EPOCH_SEC, epoch: db.epoch,
...
function account(a) {
  ...
  return { wallet: a, usdc: w.usdc, graham: w.graham, staked, index: +idx.toFixed(6), nextReward: staked * epochRate() * tier.mult, bonds, loan, borrowable, collateralUsd, health, currentLtv, maxLtv: LOOP_LTV, liqLtv: LIQ_LTV, loanApr: LOOP_APR, pendingOut,
...
http.createServer(async (req, res) => {
  ...
  if (u === '/api/config') return json(res, 200, { token: TOKEN, mint: GRAHAM_MINT, treasury: TREASURY_WALLET, live: !!GRAHAM_MINT, chainId: CHAIN_ID, rpcUrl: RPC_URL, epochSec: EPOCH_SEC, network: 'base-chain' });
  ...
  if (req.method === 'POST') {
    ...
    // real on-chain $GRAHAM balance of the connected wallet (read-only)
    if (u === '/api/balance') {
      if (!GRAHAM_MINT) return json(res, 200, { balance: 0 });
      try { const hex = await rpc('eth_call', [{ to: GRAHAM_MINT, data: '0x70a08231' + a.slice(2).toLowerCase().padStart(64, '0') }, 'latest']); return json(res, 200, { balance: Number(BigInt(hex)) / 1e18 }); }
      catch (e) { return json(res, 200, { error: 'rpc unavailable', balance: null }); }
    }

    // Direct Purchase Plan: pay USDC, receive GRAHAM at market minus the trading tax
    if (u === '/api/buy') {
      const usd = Math.max(0, Math.min(+d.amount || 0, w.usdc));
      if (usd <= 0) return json(res, 200, { error: 'enter a USDC amount' });
      const taxed = usd * (1 - TRADE_TAX);
      const got = taxed / db.marketPrice;
      w.usdc -= usd; w.graham += got; db.supply += got;
      ...
      tapePush('buy', a, got, 'GRAHAM'); save();
      return json(res, 200, { ok: true, ...account(a) });
    }
    // Enroll (stake): GRAHAM -> sGRAHAM. No maximum — bounded only by your own balance.
    // Once $GRAHAM is live (GRAHAM_MINT set), enrolling is a real on-chain transfer to the treasury:
    // the client sends the confirmed tx hash and the ledger credits sGRAHAM against that deposit.
    if (u === '/api/stake') {
      if (GRAHAM_MINT) {
        if (!/^0x[0-9a-fA-F]{64}$/.test(d.txHash || '')) return json(res, 200, { error: 'enroll sends $GRAHAM to the treasury first — missing deposit tx' });
        ...
        tapePush('enroll', a, amt, 'GRAHAM'); db.tape[0].tx = d.txHash; save();
        return json(res, 200, { ok: true, ...account(a) });
      }
      const amt = Math.max(0, Math.min(+d.amount || 0, w.graham));   // pre-launch: off-chain ledger
      if (amt <= 0) return json(res, 200, { error: 'nothing to enroll' });
      const idx = liveIndex(); const ag = amt / idx; w.graham -= amt; if (!(w.agons > 0)) w.enrolledAt = Date.now(); w.agons += ag; db.totalAgons += ag;
      tapePush('enroll', a, amt, 'GRAHAM'); save();
      return json(res, 200, { ok: true, ...account(a) });
    }
    // Redeem (unstake): sGRAHAM -> GRAHAM. Pre-launch it's an instant ledger move; once live,
    // the $GRAHAM sits in the treasury, so redemption is queued and paid out from the treasury.
    if (u === '/api/redeem') {
      ...
      if (GRAHAM_MINT) { db.withdrawals.push({ wallet: a, amount: +amt.toFixed(6), t: Date.now(), status: 'pending' }); }
      else { w.graham += amt; }
      tapePush('redeem', a, amt, 'GRAHAM'); save();
      return json(res, 200, { ok: true, queued: GRAHAM_MINT ? +amt.toFixed(6) : 0, ...account(a) });
    }
    // Bond desk: sell an asset to the protocol for discounted GRAHAM that vests. Once $GRAHAM is live,
    ...
    if (u === '/api/bond') {
      if (GRAHAM_MINT) {
        ...
        tapePush('bond', a, payout, 'GRAHAM'); db.tape[0].tx = d.txHash; save();
        return json(res, 200, { ok: true, payout, ...account(a) });
      }
      const usd = Math.max(0, Math.min(+d.amount || 0, w.usdc));   // pre-launch ledger
      if (usd <= 0) return json(res, 200, { error: 'enter a USDC amount' });
      const payout = usd / bondPrice();
      const now = Date.now();
      w.usdc -= usd; w.bonds.push({ payout, start: now, end: now + BOND_VEST_DAYS * 86400000, claimed: 0, done: false });
      db.treasury += usd; db.supply += payout;
      tapePush('bond', a, payout, 'GRAHAM'); save();
      return json(res, 200, { ok: true, payout, ...account(a) });
    }
    if (u === '/api/claim') {
      ...
      else w.graham += claimed;
      tapePush(d.autostake ? 'enroll' : 'claim', a, claimed, 'GRAHAM'); save();
      return json(res, 200, { ok: true, claimed, ...account(a) });
    }
    // Credit Desk: borrow USDC against your enrolled sGRAHAM for liquidity — your stake keeps earning.
    // The USDC is paid out from the treasury (queued); the loan accrues interest and can be repaid anytime.
    if (u === '/api/borrow') {
      ...
      if (amt <= 0) return json(res, 200, { error: 'no borrow capacity — enroll more $GRAHAM first' });
      w.loan = (w.loan || 0) + amt;
      db.withdrawals.push({ wallet: a, amount: +amt.toFixed(2), t: Date.now(), status: 'pending', kind: 'loan-usdc' });
      tapePush('borrow', a, amt, 'USDC'); save();
      return json(res, 200, { ok: true, borrowed: amt, ...account(a) });
    }
    // Loopback (Lombard): borrow USDC against sGRAHAM, buy GRAHAM, enroll — one shot
    if (u === '/api/loop') {
      const idx = liveIndex(); const staked = stakedOf(w, idx);
      const capacity = Math.max(0, staked * nav() * LOOP_LTV - (w.loan || 0));
      ...
      tapePush('loop', a, got, 'GRAHAM'); save();
      return json(res, 200, { ok: true, borrowed: borrow, looped: got, ...account(a) });
    }
    if (u === '/api/repay') {
      const amt = Math.max(0, Math.min(+d.amount || 0, Math.min(w.loan || 0, w.usdc)));
      if (amt <= 0) return json(res, 200, { error: 'nothing to repay' });
      w.usdc -= amt; w.loan = Math.max(0, (w.loan || 0) - amt); tapePush('repay', a, amt, 'USDC'); save();
      return json(res, 200, { ok: true, ...account(a) });
    }
  }
...
}).listen(PORT, () => console.log('GRAHAM ($' + TOKEN + ') on :' + PORT + ' — NAV-backed, ' + (EPOCH_SEC / 3600) + 'h epochs, R_MAX ' + (R_MAX * 100) + '%/epoch (simulated)'));
