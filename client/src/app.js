// GRAHAM app — NAV dashboard, purchase, dividend enrollment, bonds, loopback simulator.
// All state is the off-chain ledger in server/index.js. Nothing here custodies funds.
(function () {
  const $ = (id) => document.getElementById(id);
  let M = null, A = null, wallet = localStorage.getItem('graham_w') || '';
  ...
  function setWallet(a) { if (a && isW(a)) { wallet = a; localStorage.setItem('graham_w', a); renderWallet(); loadAccount(); loadChainBalance(); } else { wallet = ''; A = null; chainBal = null; localStorage.removeItem('graham_w'); renderWallet(); renderAccount(); } }
  ...
  async function loadConfig() {
    try {
      ...
      if (CFG && CFG.live) {
        ...
        const bp = document.querySelector('#buy .psub'); if (bp) bp.textContent = 'Once live, $GRAHAM trades on the open market. Buy on the DEX, then enroll it here to earn the dividend.';
        if ($('bondUnit')) $('bondUnit').textContent = 'ETH';
        if ($('bondAmt')) $('bondAmt').placeholder = '0.00 ETH';
        const bd = document.querySelector('#bond .psub'); if (bd) bd.textContent = 'Bond ETH to the treasury for discounted GRAHAM notes that vest over 2 days. Every bond grows real reserves and lifts the floor.';
      }
    } catch (e) {}
  }
  // real on-chain $GRAHAM balance, read server-side from the Base RPC
  ...
  // make sure the wallet is on Base before an on-chain transfer
  async function ensureChain() {
    ...
    try { await window.ethereum.request({ method: 'wallet_switchEthereumChain', params: [{ chainId: idHex }] }); }
      ...
      if (e && (e.code === 4902 || String(e.message).toLowerCase().includes('unrecognized') || String(e.message).includes('4902')))
        await window.ethereum.request({ method: 'wallet_addEthereumChain', params: [{ chainId: idHex, chainName: 'Base', nativeCurrency: { name: 'Ether', symbol: 'ETH', decimals: 18 }, rpcUrls: [CFG.rpcUrl], blockExplorerUrls: ['https://basechain.blockscout.com'] }] });
      else throw new Error('switch your wallet to Base to enroll');
    }
  }
  // ERC-20 transfer of $GRAHAM to the treasury; resolves with the confirmed tx hash
  async function depositGraham(amount) {
    await ensureChain();
  ...
  function renderMetrics() {
    ...
    $('mBacking').textContent = px(M.backing) + ' / GRAHAM';
    ...
    // leaderboard
    if (M.leaderboard && M.leaderboard.length) { $('lbBox').style.display = 'block'; $('lbRows').innerHTML = M.leaderboard.map((b, i) => `<div class="row"><span>${i + 1}. <b class="cd">${b.wallet}</b></span><span><b class="gold">${tok(b.staked)} sGRAHAM</b> · ${pctf(b.share)}</span></div>`).join(''); }
    // tape
    ...
    if (M.mint) { const bar = $('caBar'); if (bar) { bar.style.display = 'flex'; $('caText').textContent = M.mint.slice(0, 10) + '…' + M.mint.slice(-6); bar.href = 'https://basechain.blockscout.com/token/' + M.mint; bar.target = '_blank'; bar.title = 'Copy CA'; bar.onclick = (e) => { e.preventDefault(); navigator.clipboard && navigator.clipboard.writeText(M.mint); toast('CA copied'); }; } }
    if (M.treasuryWallet) { const tl = $('treasuryLink'); if (tl) { tl.textContent = M.treasuryWallet.slice(0, 8) + '…' + M.treasuryWallet.slice(-6); tl.href = 'https://basechain.blockscout.com/address/' + M.treasuryWallet; } }
  ...
  function drawCurve() {
    ...
    // grid
    ctx.strokeStyle = 'rgba(10,22,51,.10)'; ctx.lineWidth = 2;
    ...
    ctx.beginPath(); M.curve.forEach((pt, i) => { const x = X(pt.p), y = Y(pt.rate); i ? ctx.lineTo(x, y) : ctx.moveTo(x, y); }); ctx.strokeStyle = '#0052ff'; ctx.lineWidth = 3; ctx.stroke();
    // K marker
    ctx.strokeStyle = 'rgba(10,22,51,.28)'; ctx.setLineDash([5, 6]); ctx.beginPath(); ctx.moveTo(X(M.k), pad * 0.6); ctx.lineTo(X(M.k), h - pad); ctx.stroke(); ctx.setLineDash([]);
    // live premium dot
    const p = Math.min(M.premium, maxP); const dotX = X(p), dotY = Y(M.epochRate);
    ctx.fillStyle = '#fff'; ctx.strokeStyle = '#0052ff'; ctx.lineWidth = 3; ctx.beginPath(); ctx.arc(dotX, dotY, 9, 0, 7); ctx.fill(); ctx.stroke(); ctx.fillStyle = '#0052ff'; ctx.beginPath(); ctx.arc(dotX, dotY, 4, 0, 7); ctx.fill();
    ctx.fillStyle = 'rgba(10,22,51,.5)'; ctx.font = '600 20px "JetBrains Mono", monospace'; ctx.textAlign = 'center';
  ...
  function drawFloor() {
    ...
    ctx.strokeStyle = 'rgba(10,22,51,.09)'; ctx.lineWidth = 2;
    ...
    ctx.beginPath(); ctx.moveTo(X(0), Y(navs[0]));
    for (let i = 1; i < hist.length; i++) { ctx.lineTo(X(i), Y(navs[i - 1])); ctx.lineTo(X(i), Y(navs[i])); }
    ctx.strokeStyle = '#0052ff'; ctx.lineWidth = 3.5; ctx.lineJoin = 'round'; ctx.stroke();
    // head dot
    const lx = X(hist.length - 1), ly = Y(navs[navs.length - 1]);
    ctx.fillStyle = '#fff'; ctx.strokeStyle = '#0052ff'; ctx.lineWidth = 3; ctx.beginPath(); ctx.arc(lx, ly, 8, 0, 7); ctx.fill(); ctx.stroke();
    ctx.fillStyle = '#0052ff'; ctx.beginPath(); ctx.arc(lx, ly, 4, 0, 7); ctx.fill();
  }
  function renderAccount() {
    if (!A || !isW(wallet)) { ['aUsdg', 'aGraham', 'aStaked', 'aNext', 'aLoan'].forEach((id) => { const e = $(id); if (e) e.textContent = '—'; }); renderBonds(); return; }
    const live = CFG && CFG.live;
    // when the token is live, show the real on-chain wallet balance; otherwise the ledger balance
    $('aGraham').textContent = live ? (chainBal == null ? '…' : tok(chainBal) + ' GRAHAM') : tok(A.graham) + ' GRAHAM';
    if ($('aUsdg')) { const lbl = $('aUsdg').previousElementSibling; if (live) { $('aUsdg').textContent = A.pendingOut > 0 ? tok(A.pendingOut) + ' GRAHAM' : '—'; if (lbl) lbl.textContent = 'Pending payout'; } else { $('aUsdg').textContent = tok(A.usdc) + ' USDC'; if (lbl) lbl.textContent = 'USDC balance'; } }
    $('aStaked').textContent = tok(A.staked) + ' sGRAHAM';
    $('aNext').textContent = '+' + (A.staked * (M ? M.epochRate : 0)).toFixed(3) + ' GRAHAM';
    if ($('aLoan')) $('aLoan').textContent = tok(A.loan) + ' USDC';
  ...
  $('buyBtn').onclick = async () => {
    // once live, GRAHAM is bought on the DEX — send users to the real market
    if (CFG && CFG.live && CFG.mint) { window.open('https://dexscreener.com/base/' + CFG.mint, '_blank'); return; }
    if (!isW(wallet)) return toast('connect first'); const amt = parseFloat($('buyAmt').value); if (!(amt > 0)) return toast('enter USDC');
    const r = await post('/api/buy', { wallet, amount: amt }); if (r.error) return toast(r.error); A = r; reanchor(); renderAccount(); $('buyAmt').value = ''; toast('Purchased ' + tok(amt / M.marketPrice) + ' GRAHAM'); loadMetrics();
  };
  $('buyAmt').addEventListener('input', calcBuy);
  function calcBuy() { if (!M) return; const amt = parseFloat($('buyAmt').value) || 0; $('buyRate').textContent = '1 GRAHAM = ' + pxN(M.marketPrice) + ' USDC'; $('buyOut').textContent = tok(amt * (1 - M.tradeTax) / M.marketPrice) + ' GRAHAM'; $('buyTax').textContent = usd(amt * M.tradeTax) + ' tax (' + pctf(M.tradeTax) + ')'; }
  ...
  $('stakeMax').onclick = () => { if (!A) return; const live = CFG && CFG.live; const max = stakeMode === 'enroll' ? (live ? (chainBal || 0) : A.graham) : A.staked; $('stakeAmt').value = max > 0 ? (Math.floor(max * 100) / 100).toString() : '0'; };
  $('stakeBtn').onclick = async () => {
    ...
    if (stakeMode === 'enroll') {
      if (live) {
        if (chainBal != null && amt > chainBal + 1e-9) return toast('amount exceeds your $GRAHAM balance');
        ...
        try {
          const txHash = await depositGraham(amt);
          toast('Deposit sent — crediting sGRAHAM…');
          ...
          A = r; reanchor(); renderAccount(); $('stakeAmt').value = ''; toast('Enrolled ' + tok(amt) + ' GRAHAM (3,3)'); loadMetrics(); loadChainBalance();
      ...
      if (r.error) return toast(r.error); A = r; reanchor(); renderAccount(); $('stakeAmt').value = ''; toast('Enrolled ' + tok(amt) + ' GRAHAM'); loadMetrics(); return;
    }
    // redeem
    const r = await post('/api/redeem', { wallet, amount: amt });
    if (r.error) return toast(r.error); A = r; reanchor(); renderAccount(); $('stakeAmt').value = '';
    toast(r.queued ? 'Redemption queued — ' + tok(r.queued) + ' GRAHAM paid from treasury' : 'Redeemed ' + tok(amt) + ' GRAHAM'); loadMetrics();
  };
  ...
  $('bondBtn').onclick = async () => {
    if (!isW(wallet)) return toast('connect first');
    const live = CFG && CFG.live;
    const amt = parseFloat($('bondAmt').value); if (!(amt > 0)) return toast(live ? 'enter an ETH amount' : 'enter USDC');
    if (live) {
      if (!window.ethereum) return toast('no EVM wallet found');
      ...
      try {
        ...
        if (r.error) { btn.textContent = old; btn.disabled = false; return toast(r.error); }
        A = r; renderAccount(); $('bondAmt').value = ''; toast('Bonded — ' + tok(r.payout) + ' GRAHAM vesting'); loadMetrics();
      } catch (e) { toast(e && e.code === 4001 ? 'transaction rejected' : (e.message || 'bond failed')); }
      btn.textContent = old; btn.disabled = false; return;
    }
    const r = await post('/api/bond', { wallet, amount: amt }); if (r.error) return toast(r.error); A = r; renderAccount(); $('bondAmt').value = ''; toast('Bonded — ' + tok(r.payout) + ' GRAHAM vesting'); loadMetrics();
  };
  $('bondAmt').addEventListener('input', calcBond);
  function calcBond() {
    if (!M) return; const amt = parseFloat($('bondAmt').value) || 0; const live = CFG && CFG.live;
    if (live && M.ethUsd > 0) { const usd = amt * M.ethUsd; $('bondPrice').textContent = pxN(M.bondPrice) + ' USDC'; $('bondOut').textContent = tok(usd / M.bondPrice) + ' GRAHAM'; $('bondDisc').textContent = pctf(M.bondDiscount) + ' discount · ' + M.bondVestDays + '-day vest · deposits ETH to treasury'; }
    else { $('bondPrice').textContent = pxN(M.bondPrice) + ' USDC'; $('bondOut').textContent = tok(amt / M.bondPrice) + ' GRAHAM'; $('bondDisc').textContent = pctf(M.bondDiscount) + ' discount · ' + M.bondVestDays + '-day vest'; }
  }
  async function doClaim(autostake) { const r = await post('/api/claim', { wallet, autostake }); if (r.error) return toast(r.error); A = r; reanchor(); renderAccount(); toast(autostake ? 'Claimed & enrolled ' + tok(r.claimed) : 'Claimed ' + tok(r.claimed) + ' GRAHAM'); }

  // Credit Desk — borrow against sGRAHAM
  $('borrowBtn').onclick = async () => { if (!isW(wallet)) return toast('connect first'); const amt = parseFloat($('borrowAmt').value); if (!(amt > 0)) return toast('enter a USDC amount'); const r = await post('/api/borrow', { wallet, amount: amt }); if (r.error) return toast(r.error); A = r; reanchor(); renderAccount(); $('borrowAmt').value = ''; toast('Borrowed ' + tok(r.borrowed) + ' USDC — paid from treasury'); loadMetrics(); };
  ...
  function renderCredit() {
    ...
    const bar = $('crHealth'); if (bar) { bar.style.width = (hv * 100).toFixed(0) + '%'; bar.style.background = hv > 0.5 ? 'linear-gradient(90deg,#3fae86,#0052ff)' : hv > 0.2 ? '#d99a2b' : '#d0442e'; }
  ...
  function tick() {
    ...
    if (A && anchor.agons) $('aStaked').textContent = tok(anchor.agons * li.index) + ' sGRAHAM';
  }
